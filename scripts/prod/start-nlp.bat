...
echo [PROD] EduAI NLP/Vision Service klasorune gidiliyor: %NLP_DIR%
cd /d "%NLP_DIR%" || (
...
echo [PROD] Eski NLP/Vision servisleri kontrol ediliyor...
...
if not exist "%VENV_PYTHON%" (
    echo [PROD] [1/4] Python venv olusturuluyor...
    python -m venv venv
    if errorlevel 1 (
    ...
    echo [PROD] [1/4] Python venv zaten hazir.
)

echo [PROD] [2/4] Python bagimliliklari yukleniyor/guncelleniyor...
...
echo [PROD] [3/4] Model dosyalari kontrol ediliyor...
call :ensure_model
if errorlevel 1 exit /b 1

echo [PROD] [4/4] FastAPI Uvicorn ile baslatiliyor...
...
if "%MODEL_READY%"=="0" (
    ...
    if not errorlevel 1 (
        ...
        echo [PROD] [3/4] Model eksik veya Git LFS pointer durumda, git lfs pull deneniyor...
...
if "%MODEL_READY%"=="0" (
    ...
    echo [PROD] [3/4] Model bulunamadi, indirme URL'i uzerinden indiriliyor...
...
:install_git_lfs
echo [PROD] Git LFS bulunamadi, otomatik kurulum deneniyor...
