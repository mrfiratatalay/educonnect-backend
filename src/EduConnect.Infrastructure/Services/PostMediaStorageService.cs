...
public sealed class PostMediaStorageService(
    IWebHostEnvironment environment,
    IHttpContextAccessor httpContextAccessor,
    IOptions<AvatarUploadOptions> options) : IPostMediaStorageService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return BuildPublicUrl(userId, storedFileName);
    }
        ...
        while (true)
        {
            ...
            if (totalBytes > maxFileSizeBytes)
            {
                throw new InvalidOperationException(
                    $"Gonderi gorseli en fazla {maxFileSizeBytes / (1024 * 1024)} MB olabilir.");
            }
    ...
    private void ValidateClientMetadata(string? fileName, string? contentType, FileTypeInfo detectedFileType)
    {
        if (!string.IsNullOrWhiteSpace(fileName))
        {
            ...
            {
                throw new InvalidOperationException("Sadece JPG, PNG veya GIF gorseller yukleyebilirsiniz.");
            }
        }
    ...
    private string BuildPublicUrl(Guid userId, string storedFileName)
    {
        var httpContext = httpContextAccessor.HttpContext
            ?? throw new InvalidOperationException("Gorsel dosyasi icin genel erisim URL'si olusturulamadi.");
        ...
        var relativePath = $"{requestPath.TrimEnd('/')}/{uploadOptions.PostsFolderName}/{userId:N}/{storedFileName}";
